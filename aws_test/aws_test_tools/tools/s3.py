...
s3_list_all_objects = AWSCliTool(
    name="s3_list_all_objects",
    description="Recursively list all objects in an S3 bucket",
    content="aws s3 ls s3://$bucket_name --recursive $([[ -n \"$prefix\" ]] && echo \"--exclude '*' --include '${prefix}*'\")",
    args=[
        Arg(name="bucket_name", type="str", description="Name of the bucket to list all objects from (e.g., 'my-bucket')", required=True),
...
tool_registry.register("aws", s3_list_all_objects)
