    ...
    def execute_opal_query(self) -> ObserveCLITool:
        return ObserveCLITool(
            ...
            args=[
                ...
                Arg(name="filter", description="Filter specification. Simple: Single term (e.g., '500', 'error'). Advanced: OPAL pipeline with proper syntax. CRITICAL: Each logical term must be complete boolean expression. WRONG: 'message ~ 500 OR 502' (causes boolean error). RIGHT: 'message ~ \"500\" or message ~ \"502\"'. Use lowercase 'or'/'and'. For 5xx errors, check multiple codes: 'message ~ \"500\" or message ~ \"501\" or message ~ \"502\" or message ~ \"503\" or message ~ \"504\" or message ~ \"505\"'. For 4xx errors: 'message ~ \"400\" or message ~ \"401\" or message ~ \"403\" or message ~ \"404\"'. ONLY THESE FIELDS EXIST: timestamp, applicationName, level, loggerName, host, message, sleuthSpanId, sleuthTraceId, tags, FIELDS. NO other fields exist. HTTP codes are in 'message' field only.", required=False),
