    ...
    def execute_opal_query(self) -> ObserveCLITool:
        return ObserveCLITool(
            ...
            args=[
                ...
                Arg(name="filter", description="Filter specification - supports both simple and advanced formats. AVAILABLE FIELDS: timestamp, applicationName, level, loggerName, host, message, sleuthSpanId, sleuthTraceId, tags, FIELDS. Simple: Single term to search for (e.g., 'error', '500', 'timeout') - searches in field specified by filter_type. Advanced: Full OPAL pipeline segment (e.g., 'filter applicationName ~ \"user-service\" | filter level ~ \"ERROR\"'). EXAMPLES: HTTP 500 errors: 'filter message ~ \"500\"' or just '500'; HTTP 502/503 errors: 'filter message ~ \"502\"' or 'filter message ~ \"503\"'; Application errors: 'filter applicationName ~ \"my-app\" | filter level ~ \"ERROR\"'; Recent timeouts: 'filter message ~ \"timeout\"'; Host-specific issues: 'filter host ~ \"prod-server-1\"'. IMPORTANT: There is NO 'status' field - HTTP status codes are typically found in the 'message' field content. Use simple string matching (not regex) for status codes.", required=False),
