    ...
    def execute_opal_query(self) -> ObserveCLITool:
        return ObserveCLITool(
            ...
            args=[
                ...
                Arg(name="filter", description="Filter specification - supports both simple and advanced formats. AVAILABLE FIELDS: timestamp, applicationName, level, loggerName, host, message, sleuthSpanId, sleuthTraceId, tags, FIELDS. Simple: Single term to search for (e.g., 'error', '500', 'timeout') - searches in field specified by filter_type. Advanced: Full OPAL pipeline segment with proper boolean syntax. CORRECT OPAL SYNTAX: Use 'or' and 'and' operators with proper field comparisons. EXAMPLES: Multiple HTTP errors: 'filter message ~ \"500\" or message ~ \"502\" or message ~ \"503\"' (NOT 'message ~ 500 OR 502'); Application errors: 'filter applicationName ~ \"my-app\" and level ~ \"ERROR\"'; Field matching: 'filter host ~ \"prod-server\" and (level ~ \"ERROR\" or level ~ \"WARN\")'. WRONG: 'filter message ~ 500 OR 502' (causes 'logic expression term must be boolean' error). RIGHT: 'filter message ~ \"500\" or message ~ \"502\"'. Each comparison must be complete with field ~ \"value\". IMPORTANT: No 'status' field exists - HTTP status codes are in 'message' field content.", required=False),
                Arg(name="filter_type", description="Field to search in for simple filters only (ignored for advanced filters). Available fields: timestamp, applicationName, level, loggerName, host, message, sleuthSpanId, sleuthTraceId, tags, FIELDS. Defaults to 'message' (log content). USE CASES: 'message' (default): Search log content for HTTP status codes, error messages, stack traces; 'applicationName': Filter by specific service/app names; 'level': Filter by log levels (INFO, WARN, ERROR, DEBUG); 'host': Filter by server/container names; 'tags': Search structured tag data. For HTTP status codes, use filter_type='message' with specific codes like '500', '502', '503', or error text like 'Internal Server Error'.", required=False),
